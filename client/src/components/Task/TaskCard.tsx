...
export const TaskCard = ({ task, onUpdate }: TaskCardProps) => {
  ...
  const handleDeleteTask = () => {
    fetchEndpoint(`delete-reminder?id=${task.id}`, "GET")
      .then(() => {
      ...
      if (data.status === 0)
        toast.success(
          "Successfully completed " +
            task.name +
            "! You saved " +
            (task.incentive_max * 0.001).toFixed(4) +
            " ETH!"
        );
      else toast.error("You aren't able to check off this task.");
      onUpdate();
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {task.completed ? "✅" : task.failed ? "❌" : task.emoji || "💸"}
        </Paper>
        ...
        >
          ...
          >
              ...
              <>
                ...
                >
                  ...
                  {task.completed || task.failed ? (
                    <>
                      <s>{(task.completed || task.failed ? task.emoji + " " : "") + task.name}</s>
                      {task.category === "" ? "" : ", "}
                    </>
                  ) : (
                    <>
                      {(task.completed || task.failed ? task.emoji + " " : "") + task.name}
                      {task.category === "" ? "" : ", "}
                    </>
                  )}
                  <Typography
            ...
            >
              ...
              >
                {task.completed ? (
                  <>{(task.incentive_max * 0.001).toFixed(4)} ETH saved.</>
                ) : task.failed ? (
                  <> Lost {(task.charge * 0.001).toFixed(4)} ETH.</>
                ) : (
                  <>
                    {(task.incentive_min * 0.001).toFixed(4)} ETH -{" "}
                    {(task.incentive_max * 0.001).toFixed(4)} ETH AT STAKE
                  </>
                )}
              </Typography>
            </Box>
